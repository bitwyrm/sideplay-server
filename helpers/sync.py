...
def sync_all(
  ...
  for pl in playlists:
    tracks = get_playlist_tracks_normalized(pl, user_id, db)
    for t in tracks:
      # Remove duplicates
  ...
  # -------------------------------------------------
  resolved = {
    t["youtube_id"]: {"title": t.get("title", ""), "artists": t.get("artists", [])}
  ...
  for yt_id, meta in resolved.items():
    db.execute(
      """
        INSERT INTO tracks (youtube_id, title, artists, available)
        VALUES (?, ?, ?, 0)
        ON CONFLICT(youtube_id) DO UPDATE SET
          title = excluded.title,
          artists = excluded.artists
      """,
      (yt_id, meta.get("title", ""), json.dumps(meta.get("artists", []))),
    )
    db.execute(
  ...
  if not to_download:
    return {"ok": True, "message": "Library already complete"}

  # -------------------------------------------------
  # 5. Stream download progress and flip availability
  # -------------------------------------------------
  return StreamingResponse(stream_progress(to_download), media_type="application/jsonl")


def stream_progress(to_download: set[str]) -> Generator[str, None, None]:
  """
    ...
    for attempt in range(3):
      try:
        resp = requests.post(
          ...
          json={"youtube_ids": list(to_download)},
          stream=True,
          timeout=600,
        )
...
def get_playlist_tracks_normalized(
  playlist: Dict, user_id: str | None = None, db=None
) -> list[dict]:
  """
